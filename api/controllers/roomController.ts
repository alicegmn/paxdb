...
export const createRoom = async (req: Request, res: Response) => {
  const {
    name, description, available, air_quality, screen,
    floor, chairs, whiteboard, projector,
    temperature, activity, time, img,
  } = req.body;
...
export const updateRoom = async (req: Request, res: Response) => {
  ...
  const result = await pool.query(
    `UPDATE rooms SET 
      name = $1, description = $2, available = $3, air_quality = $4,
      screen = $5, floor = $6, chairs = $7, whiteboard = $8,
      projector = $9, temperature = $10, activity = $11,
      time = $12, img = $13
     WHERE id = $14
     RETURNING *`,
    [
...
export const deleteRoom = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await client.query('UPDATE device_configs SET room_id = NULL WHERE room_id = $1', [id]);
